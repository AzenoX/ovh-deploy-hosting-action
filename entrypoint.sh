...
sshpass -p "$OVH_HOSTING_PASSWORD" ssh -o StrictHostKeyChecking=no $OVH_HOSTING_USER@$OVH_HOSTING_DOMAIN "cd /var/www/$DOMAIN && rm -Rfv . && git clone $REPOSITORY_URL . && composer install && npm i"
echo 'Done.'