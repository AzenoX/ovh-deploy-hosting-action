...
sshpass -p "$OVH_HOSTING_PASSWORD" ssh -o StrictHostKeyChecking=no $OVH_HOSTING_USER@$OVH_HOSTING_DOMAIN "
cd /var/www/$DOMAIN
&& git fetch
...
&& npm ci
&& chmod 777 /var/www/$DOMAIN/public/projects.json
&& chmod 777 /var/www/$DOMAIN/public/projects_built.json
